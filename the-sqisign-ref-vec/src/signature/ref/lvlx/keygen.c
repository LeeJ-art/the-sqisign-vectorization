...
// int
// protocols_keygenx(public_key_t *pk, secret_key_t *sk)
// {
//     int found = protocols_keygen_p1(pk, sk);
//     protocols_keygen_p2(pk, sk);
//     return found;
// }

int
protocols_keygen_p1(public_key_t *pk, secret_key_t *sk, ec_basis_t *B_0_two)
{
    int found = 0;
    //ec_basis_t B_0_two;
...
int
protocols_keygen_p11(secret_key_t *sk, ec_basis_t *B_0_two, int found)
{
    return quat_sampling_random_ideal_O0_given_norm(
        &sk->secret_ideal, &SEC_DEGREE, 1, &QUAT_represent_integer_params, NULL);
}

int
protocols_keygen_p12(secret_key_t *sk, ec_basis_t *B_0_two, int found)
{
    return found && quat_lideal_prime_norm_reduced_equivalent(
        &sk->secret_ideal, &QUATALG_PINFTY, QUAT_primality_num_iter, QUAT_equiv_bound_coeff);
}

int
protocols_keygen_p13(secret_key_t *sk, ec_basis_t *B_0_two, int found)
{
    return found && dim2id2iso_arbitrary_isogeny_evaluation(B_0_two, &sk->curve, &sk->secret_ideal);
}
...
void
protocols_keygen_p2(public_key_t *pk, secret_key_t *sk, ec_basis_t *B_0_two)
{   
